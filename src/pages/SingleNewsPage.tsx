...
import { INewSelectorsProps } from "../componets/News/types/NewsType";
...
const SingleNewsPage: FC = () => {
  ...
  const title = getTitle(location.pathname).replace(/汉/g, "/");
  const [isHovered, setHovered] = useState(false);
  let navigate = useNavigate();

  const { fetchOneNews } = useDispatchAcions();
  const { oneNews, isLoading }: INewSelectorsProps = useTypedSelector(
    (state) => state.news
  );
